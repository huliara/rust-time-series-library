...
class Model(nn.Module):
    ...
    def forward(self, x, x_mark, y, y_mark):
        ...
        enc_PSR = enc_PSR.unfold(
            dimension=-2, size=self.patch_len, step=self.patch_len
        )  # [BC, P, D, len]
        enc_PSR = enc_PSR.reshape(
            B * C, -1, self.PSR_dim * self.patch_len
        )  # [BC, P, D*len]
...
class MDMU(nn.Module):
    def __init__(self, config):
        ...
        if config.FFT_evolve:
            self.K = sparseKernelFT1d(
                k=config.d_state,
                modes=min(16, (config.seq_len // config.patch_len) // 2 + 1),
            )
        else:
            self.K = nn.Identity()
...
class sparseKernelFT1d(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ft[:, :, :, :l] = torch.einsum(
            "bdix,iox->bdox", x_fft[:, :, :, :l], self.weights1[:, :, :l]
        )
